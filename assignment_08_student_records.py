...
def add_student(students):
    """
    Prompts the user for student details (name, ID, and list of scores)
    and appends a new dictionary record to the students list.
    """
    ...
    try:
        ...
        for i in range(1, num_scores + 1):
            ...
            # Store integer if cleanly divisible, otherwise float
            if score.is_integer():
                score = int(score)
            scores.append(score)

        # Construct dictionary record
        student = {
            "name": name,
            "id": student_id,
            "scores": scores
...
def display_all_students(students):
    """Prints a formatted table displaying all student records and averages."""
    if not students:
...
def calculate_student_average(students):
    """
    Searches for a student by ID and calculates their average score rounded to 2 decimal places.
    """
    if not students:
        print("No student records found.")
        return
...
def main():
    ...
    while True:
        ...
        print()  # Spacer line before repeating menu
